(() => {
    ...
    const furnishProjectTitle = () => {
        ...
        return match?.[1]?.trim() || scopedSpaceTitle() || 'New Project 4';
    };
    ...
    const appendFurnishStyleWidget = ({ restore } = {}) => {
        ...
        const { widget, list } = createWidget({
            index: 1,
            progressTotal: 2,
            title: 'Pick a style for the bedroom',
            subtitle: `I’ll furnish the bedroom in ${furnishProjectTitle()} in this look.`,
            showOther: false,
    ...
    const appendFurnishBudgetWidget = ({ restore } = {}) => {
        ...
        const { widget, list } = createWidget({
            index: 2,
            progressTotal: 2,
            title: 'What’s the furniture budget?',
    ...
    const appendFurnishPlan = () => {
        ...
        const project = furnishProjectTitle();
        appendMessage(
            'assistant',
            `Here’s a furnished bedroom layout for ${project} — ${furnishStyle.toLowerCase()} pieces within ${furnishBudget}. Open the floor plan to move furniture around.`
        );
        const card = document.createElement('article');
        card.className = 'spaces-home-listing-card spaces-home-plan-card';
        card.innerHTML = `
            <div class="spaces-home-plan-card-media">
                <img src="${FURNISH_PLAN_IMAGE}" alt="Bedroom floor plan">
            </div>
            <div class="spaces-home-listing-card-body">
                ...
                <div class="spaces-home-listing-card-actions">
                    <button type="button" class="spaces-home-listing-link" data-furnish-action="edit">Edit floor plan</button>
                </div>
            </div>
        `;
        const bubble = document.createElement('div');
    ...
    const beginFurnish = text => {
        ...
        appendMessage('assistant', 'A couple of details before I furnish the bedroom.');
        clearInput();
    ...
    const startFurnish = (text, options = {}) => {
        ...
        beginFurnish(text || 'Furnish the bedroom in the New Project 4');
        window.setTimeout(() => {
            starting = false;
        }, 0);
    };
    ...
    const send = text => {
        ...
        if (listingStep === 'furnish-ready') {
            appendMessage('assistant', 'Open Edit floor plan to move the furniture, or tell me what to change.');
            return true;
        }
        if (listingStep === 'link') {
